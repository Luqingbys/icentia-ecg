...
def stream_array(data, chunk_size=5, shuffle=True):
    ...
    data = data[:samples]
    data = data.reshape(-1, chunk_size, data.shape[1]) # (50, 1, 1048577)
    for i in range(data.shape[0]):
        yield data[i] # data[i]: (1, 1048577)
...
def stream_file_list(filenames, buffer_count=20, batch_size=10,
    ...
    curr_file_idx = -1
    def make_stream():
        i, filedata = next(loaded_files) # filedata: (50, 1048577)
        stream = stream_array(filedata,
                              shuffle=shuffle,
                              chunk_size=chunk_size)
        # print("Stream made.", i)
        return i, stream # stream: (5, 1048577)

    while len(streams) < buffer_count and curr_file_idx + 1 < total_files:
        try:
            curr_file_idx, stream = make_stream() # stream: (1, 1048577)
    ...
    while len(streams) > 0 or curr_file_idx + 1 < total_files:
        ...
        while len(result) < batch_size and len(streams) > 0:
            try:
                i = i % len(streams)
                next_item = next(streams[i]) # streams[i]: stream, 尺寸(1, 1048577)
                i = (i + 1) % len(streams)
                result.append(next_item) # result[i]: (1, 1048577)
        ...
        if len(result) > 0:
            if all(x.shape == result[0].shape for x in result):
                yield np.stack(result) # (len(result), 1, 1048577)
            result = []
        if shuffle:
