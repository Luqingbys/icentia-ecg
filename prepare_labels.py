...
parser.add_argument('dataset_path', default='datasets') # 数据集所在路径
parser.add_argument('-labels_file', nargs='?', default="test_labels.csv.gz") # 要保存的标签文件名，.csv
...
test_labels = []
for sample_id in range(args.start_idx, args.end_idx): # range of text examples，default 9000 ~ 11000
    filename = os.path.join(args.dataset_path, ("%05d" % sample_id) + "_batched_lbls.pkl.gz") # 如果sample_id==9000，则路径filename为 dataset_path/09000_batched_lbls.pkl.gz
    print("{}/{} ".format(sample_id, args.end_idx) + filename)
    if (not os.path.isfile(filename)):
        print(f"##### File missing: {filename}")
        continue

    segments = pickle.load(gzip.open(filename)) # 读取
    for segment_id, segment_labels in enumerate(segments):
        # print(f'sample_id: {sample_id}, segment_id: {segment_id}')
        extract_labels(sample_id, segment_id, segment_labels, test_labels=test_labels) # e.g. sample_id: 9000, segment_id: 25


data = pd.DataFrame(test_labels, columns=["sample", "segment", "frame", "btype", "rtype"]) # 创建好.csv文件框架
data.index.name = "id"
print("btype stats:", collections.Counter(data.btype))
print("rtype stats:", collections.Counter(data.rtype))
data.to_csv(args.labels_file) # 保存.csv文件
